...
export function DevicesPageClient() {
  ...
  return (
    <div className="max-w-7xl mx-auto p-3 md:p-12">
      ...
      <div className="brutal-border bg-card p-2 md:p-3 mb-4 sticky top-12 md:top-14 z-30 bg-bone">
        <input value={q} onChange={(e) => setQ(e.target.value)}
          placeholder="SEARCH DEVICES & INSTRUMENTS…"
          className="brutal-border bg-bone px-3 py-2 font-mono text-sm uppercase w-full mb-2 focus:outline-none" />
        <div className="flex flex-wrap gap-1.5 items-center">
          {CATEGORIES.map((c) => (
            <button key={c} onClick={() => setCat(c)}
              className={`brutal-border px-2.5 py-1.5 font-mono text-[11px] uppercase brutal-press ${cat === c ? "bg-acid" : "bg-bone"}`}>{c}</button>
          ))}
        ...
        <section className="mb-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            {filteredInstruments.map((i) => (
              <Link key={i.slug} href={`/mission/${i.slug}`}
        ...
        <section className="mb-6">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            {filteredDevices.map((d) => (
              <Link key={d.slug} href={`/device/${d.slug}`}
                className="brutal-border bg-card p-4 brutal-shadow-sm brutal-press block">
                <div className={`inline-block brutal-border ${CAT_COLOR[d.category] ?? "bg-bone"} px-2 py-0.5 font-mono text-[10px] uppercase`}>{d.category}</div>
      ...
      {total === 0 && <div className="brutal-border bg-hot text-bone p-4 font-mono">No matches. Try a different search or category.</div>}
