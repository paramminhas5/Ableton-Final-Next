...
const VinylIcon = () => (
  <svg viewBox="0 0 40 40" className="w-10 h-10" fill="none" stroke="currentColor" strokeWidth="2.5">
    <circle cx="20" cy="20" r="17" /><circle cx="20" cy="20" r="5" /><circle cx="20" cy="20" r="1.5" fill="currentColor" />
...
const FaderIcon = () => (
  <svg viewBox="0 0 40 32" className="w-10 h-8" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
    <line x1="8" y1="4" x2="8" y2="28" /><line x1="20" y1="4" x2="20" y2="28" /><line x1="32" y1="4" x2="32" y2="28" />
...
const TrophyIcon = () => (
  <svg viewBox="0 0 32 32" className="w-8 h-8" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
    ...
    <line x1="16" y1="24" x2="16" y2="28" /><line x1="10" y1="28" x2="22" y2="28" />
  </svg>
);
const SnakeIcon = () => (
  <svg viewBox="0 0 48 20" className="w-12 h-5" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round">
    <circle cx="4" cy="10" r="3" fill="currentColor" /><line x1="7" y1="10" x2="13" y2="10" />
    <circle cx="16" cy="10" r="3" fill="currentColor" /><line x1="19" y1="10" x2="25" y2="10" strokeDasharray="2 2" />
    <circle cx="28" cy="10" r="3" /><line x1="31" y1="10" x2="37" y2="10" strokeDasharray="2 2" />
...
const WORLD_DATA = {
  fundamentals: { label: "Fundamentals", icon: <WaveIcon />, color: "bg-acid text-ink", accent: "bg-acid", tagline: "Sound · Rhythm · Melody · Harmony · Music Tech", detail: "Everything you need to understand music before you produce or DJ. Built from learningmusic.ableton.com.", to: "/world/fundamentals" },
  dj: { label: "DJ World", icon: <VinylIcon />, color: "bg-ink text-bone", accent: "bg-volt", tagline: "Setup · Library · The Mix · Performance · Mastery", detail: "rekordbox, beatmatching, crowd reading and career. Built from the Pioneer DJ rekordbox 6.0.0 Manual.", to: "/world/dj" },
  producer: { label: "Producer", icon: <FaderIcon />, color: "bg-sun text-ink", accent: "bg-sun", tagline: "First Contact · Sound & MIDI · The Mix · Performance · Advanced", detail: "Ableton Live 12 from zero to expert. Built from the Ableton Live 12 Reference Manual.", to: "/world/producer" },
} as const;
      ...
      <section className="brutal-border border-x-0 border-t-0 bg-ink text-bone min-h-[90vh] flex flex-col justify-between relative overflow-hidden">
        ...
        <div className="brutal-border border-x-0 border-b-0 mt-16 overflow-hidden">
          <div className="flex whitespace-nowrap animate-[marquee_20s_linear_infinite] font-mono text-[10px] uppercase opacity-40 py-2">
            {Array.from({ length: 4 }).flatMap(() => ["153 MISSIONS", "·", "15 CHAPTERS", "·", "32 PATHS", "·", "FUNDAMENTALS", "·", "DJ WORLD", "·", "PRODUCER", "·", "rekordbox 6.0", "·", "Ableton Live 12", "·", "Camelot Wheel", "·", "Beat Grids", "·", "PRO DJ LINK", "·", "MIDI 2.0", "·"]).map((t, i) => <span key={i} className="px-4">{t}</span>)}
          </div>
        </div>
      </section>
      ...
      <section id="how-it-works" className="brutal-border border-x-0 border-b-0 bg-bone">
        <div className="max-w-5xl mx-auto px-4 py-16">
          ...
          <div className="mt-10 brutal-border p-5 bg-ink text-bone">
            ...
            <div className="font-mono text-[9px] opacity-40 mt-2">3 worlds · 15 chapters · 32 paths · 153 missions · quiz on every mission · trophy for every path</div>
          </div>
        </div>
      </section>
            ...
            {(["fundamentals", "dj", "producer"] as WorldTab[]).map((w) => {
              ...
              return (
                <button key={w} onClick={() => setWorldTab(w)}
                  className={`flex-1 px-4 py-3 font-mono text-[10px] uppercase brutal-press transition-colors ${worldTab === w ? (w === "dj" ? "bg-volt text-bone" : w === "producer" ? "bg-sun text-ink" : "bg-acid text-ink") : "bg-bone/5 text-bone hover:bg-bone/10"}`}>
                  {meta.label}
                </button>
              );
            })}
          ...
          <div className="space-y-0 brutal-border">
            {FAQ.map((item, i) => (
              <div key={i} className={i < FAQ.length - 1 ? "brutal-border border-x-0 border-t-0" : ""}>
                ...
                {openFaq === i && <div className="px-5 pb-5 font-mono text-sm opacity-70 leading-relaxed max-w-2xl">{item.a}</div>}
              </div>
            ))}
          </div>
        </div>
