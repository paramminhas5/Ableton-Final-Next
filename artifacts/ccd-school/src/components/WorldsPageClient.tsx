 ...
 *
 * Redesigned world cards:
 * - Large world emoji + headline hierarchy
 * - FAL AI image as right-side accent (gradient fade, not full bleed overlay)
 * - Cat mascot per world
 * - Chapter steps with labels
 * - Clean CTA footer per card
...
const WORLD_META: Record<WorldId, {
  ...
  bg: string; textColor: string; bar: string; ctaBg: string; ctaText: string;
  ...
  fundamentals: {
    ...
    bg: "bg-acid", textColor: "text-ink",
    bar: "bg-ink", ctaBg: "bg-ink", ctaText: "text-bone",
    accentBorder: "border-t-4 border-t-ink",
    catSrc: "/cats/cat-handstand.png", to: "/world/fundamentals",
    chapterBadgeDone: "bg-ink text-bone",
    chapterBadgePartial: "bg-ink/40 text-ink",
    chapterBadgeEmpty: "bg-ink/10 text-ink/40",
  },
  dj: {
    ...
    bg: "bg-ink", textColor: "text-bone",
    bar: "bg-volt", ctaBg: "bg-volt", ctaText: "text-ink",
    accentBorder: "border-t-4 border-t-volt",
    catSrc: "/cats/cat-dj.png", to: "/world/dj",
    chapterBadgeDone: "bg-volt text-ink",
    chapterBadgePartial: "bg-volt/40 text-bone",
    chapterBadgeEmpty: "bg-bone/10 text-bone/30",
  },
  producer: {
...
export function WorldsPageClient() {
  ...
  const worldStats = (world: WorldId) => {
    ...
    const chapterStats = chapters.map(ch => {
      ...
      return { slug: ch.slug, title: ch.title, number: ch.number, pct: chPct, complete: chDone === chSlugs.length && chSlugs.length > 0 };
    });
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-5 pb-24">
        {(["fundamentals", "dj", "producer"] as WorldId[]).map(world => {
          ...
          return (
            ...
            >
              {/* Main card body */}
              <div className={`${meta.bg} ${meta.textColor} relative overflow-hidden`}>

                {/* FAL AI image — right-side accent, faded left with gradient */}
                <div className="absolute inset-y-0 right-0 w-1/2 pointer-events-none">
                  <Image
                    ...
                    sizes="50vw"
                  />
                  {/* Left-to-right gradient mask so image only shows on right */}
                  <div className={`absolute inset-0 bg-gradient-to-r ${
                    world === "dj" ? "from-ink via-ink/80 to-transparent" : "from-current/0"
                  } via-current/0`}
                    style={{ background: `linear-gradient(to right, var(--tw-gradient-from) 0%, var(--tw-gradient-from) 40%, transparent 100%)` }}
                  />
                </div>
                ...
                <div className="relative z-10 p-6 md:p-8">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {/* World title — very large */}
                      <div className="flex items-center gap-3 mb-1">
                        <span className="text-5xl md:text-7xl leading-none">{meta.emoji}</span>
                        <h2 className="font-display text-4xl md:text-6xl leading-none">{meta.title.toUpperCase()}</h2>
                      </div>

                      <p className="font-display text-lg md:text-xl opacity-80 mt-1 mb-2">{meta.tagline}</p>
                      <p className="font-mono text-xs opacity-60 leading-relaxed max-w-md mb-5">{meta.description}</p>

                      {/* Chapter steps */}
                      <div className="flex flex-wrap items-center gap-1.5">
                        {stats.chapterStats.map((ch, i) => (
                          <div key={ch.slug} className="flex items-center gap-1">
                            <div className={`brutal-border px-2 py-1 flex items-center gap-1 font-mono text-[9px] uppercase transition-all ${
                              ch.complete
                              ...
                              {ch.complete ? "✓" : ch.number}
                              <span className="hidden md:inline opacity-80 ml-0.5">
                                {ch.title.split(" ")[0]}
                              </span>
                              {ch.pct > 0 && !ch.complete && (
                                <span className="opacity-60 ml-0.5">{ch.pct}%</span>
                              )}
                            </div>
                            {i < stats.chapterStats.length - 1 && (
                              <div className={`w-3 h-px opacity-30 ${world === "dj" ? "bg-bone" : "bg-ink"}`} />
                            )}
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="shrink-0 flex flex-col items-end gap-3">
                      ...
                      <div className="text-right">
                        <div className="font-display text-5xl md:text-6xl tabular-nums leading-none">{stats.pct}%</div>
                        <div className="font-mono text-[9px] uppercase opacity-60 mt-1">{stats.done}/{stats.total}</div>
                      </div>
                      {/* Cat mascot */}
                      <div
                  ...
                  {/* Progress bar */}
                  <div className={`mt-5 h-2 brutal-border overflow-hidden ${world === "dj" ? "bg-bone/10" : "bg-ink/10"}`}>
                    <div
