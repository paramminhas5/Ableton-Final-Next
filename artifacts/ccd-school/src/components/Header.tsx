...
function GoalRing({ pct, done }: { pct: number; done: boolean }) {
  ...
  return (
    <svg width="22" height="22" viewBox="0 0 22 22" aria-label={`Daily goal ${Math.round(pct * 100)}%`}>
      <circle cx="11" cy="11" r={r} fill="none" stroke="currentColor" strokeWidth="2.5" opacity="0.2" />
      <circle
  ...
  return (
    <div className="flex items-center gap-0.5" title={`${count} hearts · next in ${mm}:${ss}`}>
      {Array.from({ length: MAX_HEARTS }).map((_, i) => (
        <span key={i} className={`text-sm leading-none ${i < count ? "text-acid" : "opacity-20"}`}>♥</span>
      ))}
      {count < MAX_HEARTS && secs > 0 && (
        <span className="font-mono text-[9px] opacity-50 ml-0.5">+♥{mm}:{ss}</span>
      )}
    </div>
  );
}
...
function RankBadge({ compact }: { compact?: boolean }) {
  return null;
}
  ...
  return (
    <header className="brutal-border border-x-0 border-t-0 bg-bone sticky top-0 z-40">
      <div className="flex items-stretch h-12 md:h-14">
        <Link href="/" className="brutal-border border-y-0 border-l-0 px-3 md:px-4 flex items-center font-display text-base md:text-xl bg-acid hover-glitch shrink-0">
          CCD.SCHOOL
        </Link>
          ...
          <div className="relative flex items-stretch">
            ...
            {moreOpen && (
              <div className="absolute top-full left-0 brutal-border bg-bone min-w-[160px] z-50 shadow-lg">
                {MORE_LINKS.map((l) => (
                  <Link key={l.to} href={l.to} onClick={() => setMoreOpen(false)}
        ...
        <div className="hidden md:flex items-center gap-1.5 px-3">
          ...
          <div title={`Daily goal: ${progress.dailyXp}/${DAILY_GOAL_XP} XP`} className="flex items-center">
        ...
        <div className="md:hidden flex items-center gap-1.5 px-2">
          ...
          <span className="brutal-border bg-acid px-1.5 py-1 font-mono text-[10px]">{progress.xp}xp</span>
          <button onClick={openSearch} aria-label="Search" className="brutal-border bg-bone px-2.5 py-1.5 flex items-center">
            <SearchIcon />
          </button>
      ...
      {drawerOpen && (
        <div className="md:hidden fixed inset-0 z-50 bg-ink/80" onClick={() => setDrawerOpen(false)}>
          <div className="absolute right-0 top-0 bottom-0 w-[78%] max-w-[320px] bg-bone brutal-border border-y-0 border-r-0 overflow-y-auto animate-slide-right"
            ...
            <div className="flex items-center justify-between p-3 brutal-border border-x-0 border-t-0 bg-acid">
              ...
              <button onClick={() => setDrawerOpen(false)} className="brutal-border bg-ink text-bone px-3 py-1 font-mono text-xs">✕</button>
            </div>
            <button onClick={() => { setDrawerOpen(false); openSearch(); }}
            ...
            <nav className="flex flex-col font-mono uppercase text-sm">
              {[...PRIMARY, ...MORE_LINKS].map((l) => (
                <Link key={l.to} href={l.to} onClick={() => setDrawerOpen(false)}
...
function Marquee() {
  const items = [
    "ABLETON LIVE 12", "WARP MARKERS", "MAX FOR LIVE", "SESSION VIEW",
    "SIDECHAIN", "PUSH 3", "MIDI TRANSFORMS", "MELD", "ROAR",
    "DRUM RACK", "ABLETON LINK", "CV TOOLS",
  ];
  ...
  return (
    <div className="brutal-border border-x-0 border-b-0 bg-ink text-bone overflow-hidden">
      <div className="flex animate-marquee whitespace-nowrap py-1 font-mono text-xs uppercase tracking-widest">
        {row.map((t, i) => <span key={i} className="px-6">★ {t}</span>)}
      </div>
    </div>
  );
}
