...
const WORLD_CONFIG: Record<WorldSlug, { title: string; emoji: string; tagline: string; description: string; hero: string; accent: string; nodeActive: string; nodeInactive: string; bar: string }> = {
  ...
  dj: { title: "DJ World", emoji: "🎧", tagline: "The art of playing recorded music for people", description: "5 chapters · 10 paths · 40 missions. Source: rekordbox 6.0.0 Instruction Manual (Pioneer DJ)", hero: "bg-ink text-bone", accent: "bg-volt", nodeActive: "bg-volt text-ink", nodeInactive: "bg-bone/10 text-bone/30", bar: "bg-volt" },
  producer: { title: "Producer", emoji: "🎛", tagline: "Build music in Ableton Live 12", description: "6 chapters · 15 paths · 91 missions. Source: Ableton Live 12 Reference Manual + learningsynths.ableton.com", hero: "bg-sun text-ink", accent: "bg-sun", nodeActive: "bg-ink text-bone", nodeInactive: "bg-bone text-ink/40", bar: "bg-ink" },
};
    ...
    <main className="min-h-screen bg-bone">
      <header className={`brutal-border border-x-0 border-t-0 ${cfg.hero} relative overflow-hidden`}>
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-4 py-8 md:py-12">
          ...
          <div className="mt-6 flex items-center gap-4">
            ...
            <div className="font-mono text-xs uppercase opacity-70 shrink-0">{ws.done}/{ws.total} · {ws.pct}%</div>
            {ws.pct === 100 && <div className="font-mono text-xs uppercase shrink-0">🏆 {trophy.name}</div>}
          </div>
        </div>
      </header>
            ...
            {chapters.map((ch, i) => {
              ...
              return (
                <div key={ch.slug} className="flex items-center gap-1 shrink-0">
                  <button onClick={() => setExpandedChapter(isActive ? null : ch.slug)}
                    className={`brutal-border px-3 py-2 font-mono text-[10px] uppercase brutal-press transition-all ${stats.complete ? (world === "dj" ? "bg-volt text-ink" : "bg-ink text-bone") : isActive ? "bg-ink text-bone" : "bg-bone hover:bg-sun"}`}>
                    {stats.complete ? "✓ " : `${i + 1}. `}{ch.title}{stats.total > 0 && !stats.complete && <span className="ml-1 opacity-50">{stats.pct}%</span>}
                  </button>
                  {i < chapters.length - 1 && <div className="w-4 h-px bg-ink/20 shrink-0" />}
                </div>
              );
            })}
          </div>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-4 pb-24">
        {chapters.map((ch) => {
          ...
          return (
            <div key={ch.slug} className="brutal-border overflow-hidden">
              <button onClick={() => setExpandedChapter(isExpanded ? null : ch.slug)}
                ...
                <div>
                  <div className="font-mono text-[9px] uppercase opacity-50">CHAPTER {ch.number} · {chPaths.length} PATHS · {stats.total} MISSIONS</div>
              ...
              <div className="h-1 bg-ink/5">
                <div className={`h-full ${world === "dj" ? "bg-volt" : "bg-acid"} transition-all`} style={{ width: `${stats.pct}%` }} />
              </div>
              {isExpanded && (
                <div className="border-t border-ink/10">
                  <div className="px-4 md:px-5 py-4 bg-bone/50">
                    ...
                    {stats.complete && <div className="mt-3 brutal-border inline-flex items-center gap-2 px-3 py-1.5 bg-acid font-mono text-[10px] uppercase">🏆 {ch.trophy.name} — {ch.trophy.description}</div>}
                  </div>
                        ...
                        <div key={path.slug} className="px-4 md:px-5 py-4">
                          <div className="flex items-start justify-between gap-4 flex-wrap">
                            <div className="flex-1 min-w-0">
                              <div className="font-mono text-[9px] uppercase opacity-40">PATH {path.number} · {pTotal} MISSIONS · {pDone} DONE{path.source && <span className="ml-2 opacity-60">— {path.source.split("·")[0].trim()}</span>}</div>
                              ...
                              <div className="flex flex-wrap gap-1 mt-3">
                                {path.missionSlugs.map((s, idx) => {
                                  ...
                                  return (
                                    <Link key={s} href={`/mission/${s}`}
                                      className={`brutal-border px-2 py-0.5 font-mono text-[9px] uppercase brutal-press ${isDone ? "bg-ink text-bone" : "bg-bone hover:bg-sun"}`}>
                            ...
                            <div className="shrink-0">
                              <Link href={`/path/${path.slug}`}
                                className={`brutal-border px-4 py-2.5 font-display text-base brutal-press block text-center ${pComplete ? "bg-ink text-bone" : world === "dj" ? "bg-volt text-ink" : "bg-acid text-ink"}`}>
                                {pComplete ? "DONE ✓" : pDone > 0 ? "CONTINUE →" : "START →"}
                              </Link>
                              {pTotal > 0 && (
                                <div className="mt-2 h-1.5 brutal-border overflow-hidden w-24">
                                  <div className={`h-full ${world === "dj" ? "bg-volt" : "bg-acid"}`} style={{ width: `${pPct}%` }} />
                                </div>
                              )}
                            </div>
                          </div>
        ...
        <div className={`brutal-border p-5 ${ws.pct === 100 ? (world === "dj" ? "bg-volt text-ink" : "bg-acid text-ink") : "bg-bone opacity-40"}`}>
          ...
          {ws.pct < 100 && <div className="font-mono text-[9px] uppercase mt-2 opacity-50">Complete all {chapters.length} chapters to unlock</div>}
        </div>
      </div>
    </main>
  );
